...
class InformationDefinition extends Definition {
    ...
    EventSpecification eventSpecification   // what the information must be about
    List<ElementOfInformation> elementsOfInformation  // ORed - what element the information must contain
    List<AgentSpecification> sourceAgentSpecifications // ORed -- what a source must be
    ...
    boolean matchesAll() {
        return !elementsOfInformation && !sourceAgentSpecifications && eventSpecification.matchesAll()
    }

    MatchResult match(Bean bean, InformationAct informationAct) {
        Information info = (Information)bean
        if (!eventSpecification.matches(info.event, informationAct)) {
            return new MatchResult(matched:false, failures:["$info is not about a specified event"])
        }
        if (!sourceAgentSpecifications.any {sas -> info.sourceAgents.any {sa -> sas.matches(sa.deref(), informationAct)}}) {
