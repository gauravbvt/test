 ...
 */
abstract public class AbstractFilter implements TreeNode, Serializable, Mappable, Filter {
    ...
    private AbstractFilter parent;

    private List<AbstractFilter> children;

    //-------------------------
    public AbstractFilter() {}

    protected AbstractFilter( String collapsedText, String expandedText ) {
        this.collapsedText = collapsedText;
        this.expandedText = expandedText;
    }

    public AbstractFilter( String text ) {
        this( text, text );
    }
    ...
    public void initFromMap(Map map) {
        ...
        if (map.containsKey("children")) {
            List<AbstractFilter> list = (List<AbstractFilter>) Mapper.valueFromPersisted( map.get( "children" ) );
            setChildren( list );
        }
    }
    ...
    private void setParent( AbstractFilter parent ) {
        this.parent = parent;
    }
    ...
    public synchronized void resetChildren() {
        if ( children != null )
            for ( AbstractFilter f : children ) {
                f.resetChildren();
            }
        children = null;
    }

    public final synchronized List<AbstractFilter> getChildren() {
        if ( children == null ) {
            List<AbstractFilter> list = createChildren();
            setChildren( list );
            for ( AbstractFilter f : list ) {
    ...
    public final synchronized void setChildren( List<AbstractFilter> children ){
     ...
     */
    public final Enumeration<AbstractFilter> children() {
        return new EnumerationAdaptor<AbstractFilter>( getChildren().iterator() );
    }
    /**
     ...
     */
    abstract protected List<AbstractFilter> createChildren();
