...
/**
 * Testcase ID: MAC0020_UndoPasteAttachment
 * 	   Summary: Verify that user is able to undo the attachment which was pasted
 * @author AFour
 * 
 */
public class MAC0020_UndoPasteAttachment {
	public MAC0020_UndoPasteAttachment() {
		try {
			GlobalVariables.sTestCaseId = "MAC0020_UndoPasteAttachment";
			...
			GlobalVariables.bIsSuccess = ApplicationFunctionLibrary.login();
				if (GlobalVariables.bIsSuccess) {
					...
					GlobalVariables.oElement = GlobalVariables.oDriver.findElement(By.name("sg-editor:mo:aspect:name"));
						for (int i = 0; i <= 8; i++)
							GlobalVariables.oElement.sendKeys(Keys.BACK_SPACE);
					GlobalVariables.oElement.sendKeys(GlobalVariables.testData.get("Segment For Undo Paste Attachment"));
					// Write Results
					LogFunctions.writeLogs(GlobalVariables.sDescription);
					LogFunctions.writeResults(GlobalVariables.sTestCaseId, GlobalVariables.iStepNo, GlobalVariables.sDescription, GlobalVariables.sPassed, 
							GlobalVariables.sBlank, GlobalVariables.sBlank);
					...
					options = GlobalVariables.oDropDown.getOptions();
				    for(WebElement option : options) {
				    	if(option.getText().equals("Segment For Undo Paste Attachment")){
				    			option.setSelected();
				    			break;
				    	}
				    }
	...
	public static void main(String args[]) {
		try {
			...
			new MAC0020_UndoPasteAttachment();
