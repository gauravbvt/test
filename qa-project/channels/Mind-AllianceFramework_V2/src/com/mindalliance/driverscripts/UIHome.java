...
//				image = ImageIO.read(new File(GlobalVariables.configuration.getCurrentDir() + "//Images//Mind-Alliance_Logo.png"));
				image = ImageIO.read(new File("D:\\channels\\Mind-AllianceFramework_V2\\Images\\Mind-Alliance_Logo.png"));
			} 
			catch (Exception ex) {
...
//			GenericFunctionLibrary.initializeTestData();
			...
			// Get Browser Name
			BrowserController browsers= new BrowserController();			
		...
		try {
			if ("add".equals(e.getActionCommand())) {// when clicked on '>' button
				...
				if (jListExecute.getModel().getSize() > 0) {
					...
					File file = new File("D://Mind-AllianceFramework_V2//Reports//Index.html");
