...
@RooJpaActiveRecord( versionField = "",
                     table = "organization",
                     finders = { "findOrganizationsByNameEquals", "findOrganizationsByNameEqualsOrAcronymEquals" } )
@RooJson
...
public class Organization implements PrintableObject {
    ...
    public static Organization findOrCreate( String orgName, Calendar submitted ) {
        if ( orgName == null )
            throw new IllegalArgumentException( "Organization name can't be null" );
        Organization org;
        List<Organization> organizations = findOrganizationsByNameEqualsOrAcronymEquals( orgName,
                                                                                         orgName ).getResultList();
        if ( organizations.isEmpty() ) {
            LOG.debug( "Creating organization {}", orgName );
            org = new Organization();
            org.setName( orgName );
            org.setAdded( submitted );
            org.persist();
        } else
    ...
    public static long getLastModified() {
        return entityManager().createQuery( "SELECT max(added) FROM Organization", Calendar.class )
            .getSingleResult()
            .getTimeInMillis();
    }
