 ...
 * Time: 10:30:08 AM
 */
public class ModelClassificationSystemsPanel extends AbstractCommandablePanel {
    ...
    public ModelClassificationSystemsPanel( String id ) {
        super( id );
        init();
    }
    ...
    private void init() {
        canBeEdited = getCollaborationModel().isDevelopment() && isLockedByUser( Channels.ALL_CLASSIFICATIONS );
    ...
    public List<String> getClassificationSystems() {
        ...
        for ( Classification classification : getCollaborationModel().getClassifications() ) {
    ...
    private void deleteSystem( String systemName ) {
        ...
        multiCommand.setChange( new Change( Change.Type.Updated, getCollaborationModel(), "classifications") );
        multiCommand.makeUndoable( false );
        CollaborationModel collaborationModel = getCollaborationModel();
        for ( Classification classification : collaborationModel.classificationsFor( systemName ) ) {
    ...
    private boolean isReferenced( String systemName ) {
        ...
        return CollectionUtils.exists(
                getCollaborationModel().classificationsFor( systemName ),
                new Predicate() {
                    @Override
