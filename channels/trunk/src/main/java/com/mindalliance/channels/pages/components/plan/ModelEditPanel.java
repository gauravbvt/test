...
import com.mindalliance.channels.pages.components.plan.menus.ModelEditActionsMenuPanel;
 ...
 * Time: 5:08:07 PM
 */
public class ModelEditPanel extends AbstractFloatingMultiAspectPanel {
    ...
    public ModelEditPanel( String id, IModel<? extends Identifiable> iModel, Set<Long> expansions ) {
        super( id, iModel, expansions );
    }

    public ModelEditPanel( String id, IModel<? extends Identifiable> iModel,
    ...
    protected List<? extends GeoLocatable> getGeoLocatables() {
        ...
        Place planLocale = getCollaborationModel().getLocale();
    ...
    protected MenuPanel makeActionMenu( String menuId ) {
        return new ModelEditActionsMenuPanel(
                menuId,
    ...
    protected Component makeAspectPanel( String aspect, Change change ) {
     ...
     private Component getPlanDetailsPanel() {
        return new ModelEditDetailsPanel( "aspect", getModel(), getExpansions() );
    }
    ...
    public void updateWith( AjaxRequestTarget target, Change change, List<Updatable> updated ) {
        if ( change.isForInstanceOf( Issue.class )
                && change.getSubject( getCommunityService() ) instanceof CollaborationModel )
            setAspectShown( target, DETAILS );
        if ( !( change.isSelected() && ( change.isForInstanceOf( CollaborationModel.class ) ) ) ) {
