 ...
 * Time: 4:26:10 PM
 */
public class ModelMapPng extends DiagramPng {
    /**
     * The log.
     */
    private static final Logger LOG = LoggerFactory.getLogger( ModelMapPng.class );
    /**
    ...
    protected Diagram makeDiagram( double[] size, 
        ...
        if ( parameters.getNamedKeys().contains( "group" )
            ...
            try {
                group = modelService.find( ModelEntity.class, groupId );
        ...
        if ( parameters.getNamedKeys().contains( "segment" )
            ...
            try {
                segment = modelService.find( Segment.class, segmentId );
            } catch ( NotFoundException e ) {
        ...
        List<Segment> allSegments = modelService.list( Segment.class );
        return diagramFactory.newModelMapDiagram(
                allSegments,
                groupByPhase,
                groupByEvent,
                group,
