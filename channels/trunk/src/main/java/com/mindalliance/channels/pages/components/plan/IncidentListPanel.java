 ...
 * Time: 1:43:30 PM
 */
public class IncidentListPanel extends AbstractCommandablePanel {
    /**
     * Collator.
     */
     ...
     * Incidents container.
     */
    private WebMarkupContainer incidentsDiv;

    public IncidentListPanel( String id, IModel<? extends Identifiable> model, Set<Long> expansions ) {
        super( id, model, expansions );
        init();
    }

    private void init() {
        incidentsDiv = new WebMarkupContainer( "incidentsDiv" );
        incidentsDiv.setOutputMarkupId( true );
        add( incidentsDiv );
        incidentsDiv.add( makeIncidentsTable() );
    }

    private ListView<EventWrapper> makeIncidentsTable() {
        List<EventWrapper> eventWrappers = getWrappedIncidents();
        return new ListView<EventWrapper>( "incident", eventWrappers ) {
            /** {@inheritDoc} */
            protected void populateItem( ListItem<EventWrapper> item ) {
                addEventNameCell( item );
                addConfirmedCell( item );
                addShowEventCell( item );
            }
        };
    }

    private void addEventNameCell( final ListItem<EventWrapper> item ) {
        ...
        Label nameLabel = new Label( "name-label", new PropertyModel<String>( wrapper, "name" ) );
    ...
    private void addConfirmedCell( ListItem<EventWrapper> item ) {
        ...
        confirmedCheckBox.add( new AjaxFormComponentUpdatingBehavior( "onchange" ) {
            protected void onUpdate( AjaxRequestTarget target ) {
                incidentsDiv.addOrReplace( makeIncidentsTable() );
                target.addComponent( incidentsDiv );
    ...
    private void addShowEventCell( ListItem<EventWrapper> item ) {
        final EventWrapper wrapper = item.getModel().getObject();
        Link link = new AjaxFallbackLink( "show-link" ) {
            public void onClick( AjaxRequestTarget target ) {
                update( target, new Change( Change.Type.Expanded, wrapper.getEvent() ) );
            }
        };
        makeVisible( link, !wrapper.isMarkedForCreation() );
        item.add( link );
    }

    /**
     * Get incident wrapperss.
     *
     * @return a list of plan event wrappers
     */
    public List<EventWrapper> getWrappedIncidents() {
        // Existing incidents
        List<EventWrapper> wrappers = new ArrayList<EventWrapper>();
        for ( Event incident : getPlan().getIncidents() ) {
            wrappers.add( new EventWrapper( incident, true ) );
        }
