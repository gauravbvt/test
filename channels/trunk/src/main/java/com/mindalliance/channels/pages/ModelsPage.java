 ...
 * Time: 11:22 AM
 */
public class ModelsPage extends AbstractChannelsBasicPage {

    /**
     * Class logger.
     */
    private static final Logger LOG = LoggerFactory.getLogger( ModelsPage.class );
    ...
    public ModelsPage( PageParameters parameters ) {
        super( parameters );
    }
    ...
    private void addModelSelector() {
        List<CollaborationModel> allCollaborationModels = getModelManager().getModelsReadableBy( getUser() );
        DropDownChoice<CollaborationModel> planSelector = new DropDownChoice<CollaborationModel>(
                "modelChoice",
                new PropertyModel<CollaborationModel>( this, "selectedModel" ),
                allCollaborationModels,
        ...
        planSelector.setVisible( allCollaborationModels.size() > 1 );
        getContainer().addOrReplace( planSelector );
    }

    public CollaborationModel getSelectedModel() {
        return getCollaborationModel();
    }

    public void setSelectedModel( CollaborationModel collaborationModel ) {
    ...
    private void addModelName() {
        getContainer().add( new Label( "modelName", getCollaborationModel().getVersionedName() ) );
    }

    private void addModelClient() {
        getContainer().add( new Label( "modelClient", getCollaborationModel().getClient() ) );
    }
    ...
    private void addAttachments() {
        ...
        boolean visible = !isAttachmentsReadOnly() || !getCollaborationModel().getAttachments().isEmpty();
        makeVisible( attachmentsLabel, visible );
        AttachmentPanel attachmentPanel = new AttachmentPanel(
                "attachments",
                new Model<ModelObject>( getCollaborationModel() ),
    ...
    public List<Attachment> getReferences() {
        return (List<Attachment>) CollectionUtils.select(
                getCollaborationModel().getAttachments(),
                new Predicate() {
                    @Override
    ...
    private void addGotoLinks( CommunityService communityService, ChannelsUser user ) {
        ...
        BookmarkablePageLink<? extends WebPage> gotoRFIsLink =
                getRFIsLink( "gotoRFIs", getCollaborationModel(), true );
        ...
        BookmarkablePageLink<? extends WebPage> gotoFeedbackLink =
                getFeedbackLink( "gotoFeedback", getCollaborationModel(), true );
        ...
        // Model editor link
        BookmarkablePageLink gotoModelLink = newTargetedLink( "gotoModel", "", ModelPage.class, null, collaborationModel );
        if ( isInCommunityContext() ) {
        ...
        gotoIconsContainer.add(
                ...
                new WebMarkupContainer( "model" )
                        .add( gotoModelLink )
                        .setVisible( user.hasAccessTo( collaborationModel.getUri() ) || collaborationModel.isVisibleToUsers() )
                ...
                new WebMarkupContainer( "issues" )
                        .add( gotoIssuesLink )
                        .setVisible( user.hasAccessTo( collaborationModel.getUri() ) || collaborationModel.isViewableByAll() )
                        .setOutputMarkupId( true )
        );
