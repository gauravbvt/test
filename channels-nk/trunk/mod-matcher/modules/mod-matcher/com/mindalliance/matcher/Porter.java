...
/**
 * Implementation of Porter stemming alogrithm, shamelessly copied from http://ir.dcs.gla.ac.uk/resources/linguistic_utils/porter.java
 * <p/>
 * author:   Fotis Lazarinis (actually I translated from C to Java) date:     June 1997 address:  Psilovraxou 12,
 * Agrinio, 30100
 * <p/>
 * comments: Compile it, import the Porter class into you program and create an instance. Then use the stripAffixes
 * method of this method which takes a String as input and returns the stem of this String again as a String.
 */
public class Porter {
    ...
    private boolean cvc( String str ) {
        ...
        if ( ( !vowel( str.charAt( length - 1 ), str.charAt( length - 2 ) ) ) && ( str.charAt( length - 1 ) != 'w' )
             && ( str.charAt( length - 1 ) != 'x' ) && ( str.charAt( length - 1 ) != 'y' ) && ( vowel(
                str.charAt( length - 2 ), str.charAt( length - 3 ) ) ) ) {
        ...
        if ( hasSuffix( str, "eed", stem ) ) {
            ...
            if ( ( hasSuffix( str, "ed", stem ) ) || ( hasSuffix( str, "ing", stem ) ) ) {
                if ( containsVowel( stem.str ) ) {
                    ...
                    if ( ( hasSuffix( str, "at", stem ) ) || ( hasSuffix( str, "bl", stem ) ) || ( hasSuffix( str, "iz",
                                                                                                              stem ) ) ) {
                        str += "e";
                    } else {
                        int length = str.length();
    ...
    private String step2( String str ) {
        ...
        String[][] suffixes = {
                ...
                { "iser", "ize" }, { "abli", "able" }, { "alli", "al" }, { "entli", "ent" }, { "eli", "e" },
    ...
    private String step3( String str ) {
        ...
        String[][] suffixes = {
                { "icate", "ic" }, { "ative", "" }, { "alize", "al" }, { "alise", "al" }, { "iciti", "ic" },
                { "ical", "ic" }, { "ful", "" }, { "ness", "" } };
        NewString stem = new NewString();
    ...
    private String step4( String str ) {
        ...
        String[] suffixes = {
                "al", "ance", "ence", "er", "ic", "able", "ible", "ant", "ement", "ment", "ent", "sion", "tion", "ou",
    ...
    private String step5( String str ) {
        ...
        if ( str.length() == 1 )
            return str;
        if ( ( str.charAt( str.length() - 1 ) == 'l' ) && ( str.charAt( str.length() - 2 ) == 'l' ) && ( measure( str )
                                                                                                         > 1 ) )
            if ( measure( str ) > 1 ) {/* measure(str)==measure(stem) if ends in vowel */
                String tmp = "";
                for ( int i = 0; i < str.length() - 1; i++ )
